                                    ...
                                    <div class="card-body" style="padding:5px; padding-bottom:0px">
                                        <table class="table table-bordered" id="dataTable" width="100%" cellspacing="0">
                                            <thead>
                                                <tr class="thead-light">
                                                    ...
                                                    {{ $transaction->shop->nama_toko }}
                                                        <span class="float-right mt-1">{{ $transaction->transaction_status }}</span>
                                                    </th>
                                                </tr>
                                            </thead>
                                            <tbody>
                                            ...
                                            @endforeach
                                                <tr>
                                                    <td>
                                                        @if($transaction->payment_system == 'DP' && $transaction->transaction_status == 'BELUM BAYAR' || $transaction->payment_system == 'DP' && $transaction->transaction_status == 'DIBATALKAN' || $transaction->payment_system == 'DP' && $transaction->transaction_status == 'SEDANG DIPROSES' || $transaction->payment_system == 'DP' && $transaction->transaction_status == 'MENUNGGU KONFIRMASI' || $transaction->payment_system == 'DP' && $transaction->transaction_status == 'TAGIHAN')
                                                        {{--  Total Pesanan  --}}
                                                            <span class="float-right">Total Pesanan: Rp{{ number_format($transaction->total_price) }}
                                                            </span>
                                                            <br>
                                                            @if($transaction->transaction_status == 'BELUM BAYAR' || $transaction->transaction_status == 'DIBATALKAN')
                                                                <span class="float-right">Jumlah DP: <span
                                                                    class="font-weight-bold">Rp{{ number_format($transaction->downpayment) }}</span>
                                                                </span>
                                                            @elseif($transaction->payment_system == 'DP' && $transaction->transaction_status == 'SEDANG DIPROSES' || $transaction->payment_system == 'DP' && $transaction->transaction_status == 'MENUNGGU KONFIRMASI')
                                                                <span class="float-right">Tagihan Berikutnya: <span
                                                                    class="font-weight-bold">Rp{{ number_format($transaction->tagihan) }}</span>
                                                                </span>
                                                            
                                                            @elseif ($transaction->transaction_status == 'TAGIHAN')
                                                                <span class="float-right">Jumlah Tagihan: <span
                                                                    class="font-weight-bold">Rp{{ number_format($transaction->tagihan) }}</span>
                                                                </span>
                                                            @endif
                                                        @elseif($transaction->payment_system == 'Cash' && $transaction->transaction_status == 'BELUM BAYAR')
                                                            <span class="float-right">Jumlah Harus Dibayar: <span
                                                        ...
                                                        @if($transaction->transaction_status == 'BELUM BAYAR' || $transaction->transaction_status == 'TAGIHAN' || $transaction->transaction_status == 'SEDANG DIPROSES' || $transaction->transaction_status == 'MENUNGGU KONFIRMASI')
                                                            {{--  Button Batalkan Pesanan  --}}
                                                            {{--  <form class="float-right mt-2" action="{{ route('batalkan-pesanan') }}" method="POST">
                                                            @csrf
                                                                <input type="hidden" name="idTransaksi" value="{{ $transaction->id }}">
                                                                <button type="submit" class="btn btn-danger btn-sm">Batal Pesanan</button>
                                                            </form>  --}}

                                                            <!-- Button Batalkan Pesanan modal -->
                                                            <button type="button" class="btn btn-sm btn-danger float-right mt-2" data-toggle="modal" data-target="#batalPesananModal{{ $transaction->id }}">
                                                            Batalkan Pesanan
                                                            </button>
                                                            @if($transaction->transaction_status == 'BELUM BAYAR' || $transaction->transaction_status == 'TAGIHAN')
                                                        ...
                                                        @elseif ($transaction->transaction_status == 'DIKIRIM')
                                                            ...
                                                            <form class="float-right mt-2" action="{{ route('pesanan-diterima') }}" method="POST">
                                                            @csrf
                                                                <input type="hidden" name="idTransaksi" value="{{ $transaction->id }}">
                                    ...
                                    <tr>
                                        <td style="padding: 0;" class="d-flex flex-column">
                                            <div class="clearfix">
                                                ...
                                                @if($transaction->payment_system == 'DP' && $transaction->transaction_status == 'BELUM BAYAR' || $transaction->payment_system == 'DP' && $transaction->transaction_status == 'DIBATALKAN' || $transaction->payment_system == 'DP' && $transaction->transaction_status == 'SEDANG DIPROSES' || $transaction->payment_system == 'DP' && $transaction->transaction_status == 'MENUNGGU KONFIRMASI' || $transaction->transaction_status == 'TAGIHAN')
                                                    <span class="float-right small text-muted"> Total Pesanan: Rp{{ number_format($transaction->total_price) }} </span>
                                                @endif
                                                @if($transaction->payment_system == 'DP' && $transaction->transaction_status == 'BELUM BAYAR' || $transaction->transaction_status == 'DIBATALKAN')
                                                    <span class="float-right small text-muted">
                                                        Jumlah DP: <span class="font-weight-bold">Rp{{ number_format ($transaction->downpayment) }}</span>
                                                    </span>
                                                @elseif ($transaction->transaction_status == 'TAGIHAN')
                                                    <span class="float-right small text-muted">
                                                        Jumlah Tagihan: <span class="font-weight-bold">Rp{{ number_format($transaction->tagihan) }}</span>
                                                    </span>
                                                @elseif($transaction->payment_system == 'DP' && $transaction->transaction_status == 'SEDANG DIPROSES' || $transaction->payment_system == 'DP' && $transaction->transaction_status == 'MENUNGGU KONFIRMASI')
                                                    <span class="float-right small text-muted">
                                                        Tagihan Berikutnya: <span class="font-weight-bold">Rp{{ number_format($transaction->tagihan) }}</span>
                                                    </span>
                                                @elseif ($transaction->transaction_status == 'DIKIRIM' || $transaction->transaction_status == 'SELESAI' || $transaction->payment_system == 'Cash' && $transaction->transaction_status == 'SEDANG DIPROSES' || $transaction->payment_system == 'Cash' && $transaction->transaction_status == 'MENUNGGU KONFIRMASI' || $transaction->payment_system == 'Cash' && $transaction->transaction_status == 'DIBATALKAN')
                                                    <span class="float-right small text-muted">
                                                        Total Pesanan: <span class="font-weight-bold">Rp{{ number_format($transaction->total_price) }}</span>
                                                    </span>
                                                @elseif($transaction->payment_system == 'Cash' && $transaction->transaction_status == 'BELUM BAYAR')
                                                    <span class="float-right small text-muted">
                                                        Jumlah Harus Dibayar: <span class="font-weight-bold">Rp{{ number_format($transaction->total_price) }}</span>
                                                    </span>
                                                @endif
                                            </div>
                                            <br>
                                            <div class="clearfix">
                                                ...
                                                @if($transaction->transaction_status == 'BELUM BAYAR' || $transaction->transaction_status == 'TAGIHAN')
                                                    ...
                                                    <div class="dropdown-menu dropdown-transaksi mr-2">
                                                        ...
                                                        {{--  Button Batalkan Pesanan  --}}
                                                        {{--  <form action="{{ route('batalkan-pesanan') }}" method="POST">
                                                        @csrf
                                                            <input type="hidden" name="idTransaksi" value="{{ $transaction->id }}">
                                                            <button type="submit" class="btn btn-block btn-dropdown btn-dropdown-transaksi btn-sm text-left">Batal Pesanan</button>
                                                        </form>  --}}
                                                ...
                                                @else
                                                    {{--  Button Detail Pesanan  --}}
                                                    <form class="float-right " action="{{ route('detail-pesanan') }}" method="POST">
                                                    @csrf
                                                        <input type="hidden" name="idTransaksi" value="{{ $transaction->id }}">
                                                        <button type="submit" class="btn btn-info btn-sm">Detail</button>
                                                    </form>
                                                    @if($transaction->transaction_status == 'DIKIRIM')
                                                        {{--  Button Pesanan Diterima  --}}
                                                        <form class="float-right" action="{{ route('pesanan-diterima') }}" method="POST">
                                                        @csrf
                                                            <input type="hidden" name="idTransaksi" value="{{ $transaction->id }}">
                                                            <button type="submit" class="btn btn-success btn-sm">Pesanan Diterima</button>
                                                        </form>
                                                    @elseif($transaction->transaction_status == 'SEDANG DIPROSES' || $transaction->transaction_status == 'MENUNGGU KONFIRMASI')
                                                        {{--  Button Batalkan Pesanan  --}}
                                                        {{--  <form class="float-right mr-1" action="{{ route('batalkan-pesanan') }}" method="POST">
                                                        @csrf
                                                            <input type="hidden" name="idTransaksi" value="{{ $transaction->id }}">
                                                            <button type="submit" class="btn btn-danger btn-sm">Batal Pesanan</button>
                                                        </form>  --}}
                                                        ...
                                                        <!-- Button Batalkan Pesanan modal -->
                                                        <button type="button" class="btn btn-sm btn-danger float-right mr-1" data-toggle="modal" data-target="#batalPesananModal{{ $transaction->id }}">
                                                        Batalkan Pesanan
                                                        </button>
                                                    @endif
                                                @endif
                    ...
                    @csrf
                        <div class="modal-body">
                            ...
                            <div class="row">
                                ...
                                @elseif($transaction->transaction_status == 'MENUNGGU KONFIRMASI')
                                    <div class="col-lg-12 text-center">
                                        <h6 class="font-weight-bold text-secondary mx-auto">Yakin Ingin Membatalkan Pesanan Ini?</h6>
                                ...
                                @else
                                    ...
                                    @else
                                        @if(isset($transaction->tgl_byr_dp) && !isset($transaction->tgl_byr_tagihan) || $transaction->transaction_status == 'TAGIHAN')
                                            <div class="col-lg-12 text-center">
                                                <h6 class="font-weight-bold text-secondary mx-auto">Yakin Ingin Membatalkan Pesanan Ini?</h6>
